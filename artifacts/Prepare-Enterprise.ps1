...
param(
    ...
    [ValidateSet("EnterpriseSubordinateCA","EnterpriseSubordinateCA")]
    [string]$CAType,
...
Assert-AllowedValue -Name "CAType" -Value $CAType -Allowed @("EnterpriseSubordinateCA","EnterpriseSubordinateCA")
...
"@
    ...
    if (-not $caFeature.Installed) {
        ...
        if (-not (Get-WindowsFeature -Name RSAT-ADCS-Mgmt).Installed) {
            Install-WindowsFeature -Name RSAT-ADCS-Mgmt | Out-Null
        }
